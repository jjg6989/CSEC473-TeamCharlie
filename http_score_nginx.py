...
def score_NGINX(queue, alive, lock, target, port, value, team_num):
    while alive():
        try:
            ...
            if res.ok:
                lock.acquire()
                queue.put({'service': 'NGINX', 'status': 'UP', 'host': target, 'value': value, 'team_num': team_num})
                lock.release()
            # We got a non-200 response code
            else:
                lock.acquire()
                queue.put({'service': 'NGINX', 'status': 'DOWN', 'host': target, 'value': value, 'team_num': team_num})
                lock.release()
